...
type _GenerateNestedInput<TFields, TBreakpoints extends string = Breakpoints> =
    {
        ...
        ? Omit<BackpackCore['BackpackToggle']['Input'], 'default_value'> & {
            default_value: {
                [key in TBreakpoints]?: TFields[Field]['Output'];
            };
        }
        ...
        : TFields[Field] extends BackpackCore['BackpackPositionAlign']
            ? Omit<BackpackCore['BackpackPositionAlign']['Input'], 'default_value'> & {
            default_value: {
                [key in TBreakpoints]?: TFields[Field]['Output'];
            };
        }
        : TFields[Field] extends BackpackCore['BackpackColorPicker']
        ? Omit<BackpackCore['BackpackColorPicker']['Input'], 'default_value'> & {
            default_value: {
                [key in TBreakpoints]?: TFields[Field]['Output'];
            };
        }
        : TFields[Field] extends BackpackCore['BackpackNumber']
            ? BackpackCore['BackpackNumber']['Input'] & {
            default_value: {
                [key in TBreakpoints]?: TFields[Field]['Output'];
            };
        }
        : TFields[Field] extends BackpackCore['BackpackText']
            ? BackpackCore['BackpackText']['Input'] & {
            default_value: {
                [key in TBreakpoints]?: TFields[Field]['Output'];
            };
        }
        : TFields[Field] extends BackpackCore['BackpackOption']
            ? TFields[Field]['Input'] & {
            default_value: {
                [key in TBreakpoints]?: TFields[Field]['Output'];
            };
...
type _GenerateInput<TFields, TBreakpoints extends string = Breakpoints> = {
    [Field in keyof TFields]: TFields[Field] extends BackpackBreakpointsPredicate
        ...
        : TFields[Field] extends BackpackCore['BackpackTheme']
            ? TFields[Field]['Input'] & {
            default_value?: TFields[Field]['Output'];
        }
        : TFields[Field] extends BackpackCore['BackpackColorPicker']
